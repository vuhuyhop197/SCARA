...
#define En_X_Pin	GPIO_PIN_3
#define	En_X_Port	GPIOE
#define En_Y_Pin 	GPIO_PIN_4
#define En_Y_Port GPIOD
#define	Start 	1
#define Stop 		0
static char * request_commands_arry[__CMD_COUNT] = {"UNKNOWN", "ADD", "GET", "SET", "RESET","GRAP","ORT","HOME"};
static char * request_params_arry[__PARAM_COUNT] = {"UNDEFINED", "ALL", "TARGETPOSITION", "CURRENTPOSITION", "MINSPS", "MAXSPS", "CURRENTSPS", "ACCSPS", "ACCPRESCALER", "STATUS","TAKE","DROP"};
        ...
        case PARAM_TARGETPOSITION:
				...
				target_pos= value;
					if (stepper=='X'){
						...
						HAL_GPIO_WritePin(En_Z_Port,En_Z_Pin,Start);
					}
					return Stepper_SetTargetPosition(stepper, value);
        case PARAM_CURRENTPOSITION:
					return Stepper_SetCurrentPosition(stepper, value);
        case PARAM_MINSPS:         
					return Stepper_SetMinSPS(stepper, value);
        case PARAM_MAXSPS:         
					return Stepper_SetMaxSPS(stepper, value);
  ...
  if (stepper == '\0') {
    ...
    switch (command) {
      ...
      case CMD_GET:
        ...
        break;
			case CMD_Grap:
				if(parameter == PARAM_TAKE)
				Grap_object(1);
				if(parameter == PARAM_DROP)
				Grap_object(0);
				break;
			case CMD_Orient:
				Set_Orient(value);
