...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart){
  ...
  if (serialStatus & SERIAL_TXOVERFLOW) {
      ...
      if (serialStatus & SERIAL_TXOVERFLOWMSG) {
        ...
        while(HAL_UART_Transmit_DMA(&huart4, (uint8_t *)TX_OVERFLOW_MSG, strlen(TX_OVERFLOW_MSG)) == HAL_BUSY) { __HAL_UNLOCK(&huart4); }
        return;
      }
  }
...
void Serial_ExecutePendingTransmits(void) {
  ...
  while(HAL_UART_Transmit_DMA(&huart4, (uint8_t *)txOutPtr, txLength) == HAL_BUSY) { __HAL_UNLOCK(&huart4); }
  serialStatus |= SERIAL_TX;
...
void Serial_InitRxSequence(void) {
  while(HAL_UART_Receive_DMA(&huart4, rxBuffer, RX_BUFFER_SIZE) == HAL_BUSY) { __HAL_UNLOCK(&huart4); }
  serialStatus |= SERIAL_RX;
}
...
void Serial_CheckRxTimeout(void) {
  ...
  bytesTransfered = (RX_BUFFER_SIZE - huart4.hdmarx->Instance->NDTR);
