...
void SetStepTimerByCurrentSPS(stepper_state * stepper){
  if (stepper -> STEP_TIMER != NULL && stepper -> STEP_TIMER -> Instance != NULL){
    ...
    timer -> ARR = timerTicks;
		//__HAL_TIM_SET_COMPARE(stepper -> STEP_TIMER, stepper->STEP_CHANNEL, 100);
		if(stepper -> name == 'X'){
			timer -> CCR3 = timerTicks/2;
		}
		else if(stepper -> name == 'Y'){
			timer -> CCR1 = timerTicks/2;
		}
		else if(stepper -> name == 'Z'){
			timer -> CCR1 = timerTicks/2;
		}
		//timer-> CNT=timerTicks/2;
  }
}
...
//			value = Max_X_Pos;
//		}
//		else if(value < - Max_X_Pos){
//			value = - Max_X_Pos;
//		}
//	}
//	else if(stepperName=='Y'){
//		if(value > (Max_Y_Pos+Max_X_Pos))
//		{
//			value = (Max_Y_Pos+Max_X_Pos);
//		}
//		else if(value < - (Max_Y_Pos+Max_X_Pos)){
//			value = - (Max_Y_Pos+Max_X_Pos);
//		}
//	}
  if (stepper == NULL)
